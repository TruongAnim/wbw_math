...
class Scene1(MyScene):
    def construct(self):
        ...
        text1 = brace1.get_tex("{:.4f}".format((BC.get_length())))
        text2 = brace2.get_tex("{:.4f}".format((AB.get_length())))
        text3 = brace3.get_tex("{:.4f}".format((AC.get_length())))
        ...
        self.my_play(FadeIn(pi2, shift=LEFT * 2))
        self.my_play(NumberCreatureSays(pi2,
        ...
        self.my_play(FadeTransform(pi, pi_wonder), FadeTransform(text_heron, text_really))
...
class Scene2(MyScene):
    def construct(self):
        ...
        brace2 = Brace(AB, AB.copy().rotate(-PI / 2).get_unit_vector())
        brace3 = Brace(AC, AC.copy().rotate(PI / 2).get_unit_vector())
        text1 = brace1.get_tex("a", "=", "{:.4f}".format((BC.get_length())))
        text2 = brace2.get_tex("b", "=", "{:.4f}".format((AB.get_length())))
        text3 = brace3.get_tex("c", "=", "{:.4f}".format((AC.get_length())))
        color_map = {
            text1: RED,
            text2: GREEN,
            text3: BLUE
        ...
        s = MathTex(r"s", "={{", "a", "+", "b", "+", "c", r"}\over 2}",
                    "=", "5.0684").shift(DOWN * 2 + LEFT * 2).scale(0.8)
        color_map_2 = {
            "a": RED,
            "b": GREEN,
            "c": BLUE,
        ...
        self.my_play(Write(s[8]), Write(s[9]))
        self.wait()
        area = MathTex(r"\text{Area}=", "\sqrt{", "s", "(", "s", "-", "a", ")(",
                       "s", "-", "b", ")(", "s", "-", "c", ")}", "=", "4.4").scale(0.8)
...
class Scene3(MyScene):
    def construct(self):
        ...
        self.my_play(Create(triangle), Create(AH))
        fadein_map = (
            ((brace1, text1), (brace2, text2), (brace3, text3)),
            (UP, DR, DL)
        )
        ...
        self.my_play(Write(table), *[
            Write(content[i]) for i in range(0, 11, 2)
        ])
        self.my_play(Wiggle(content[8]))
        self.my_play(Wiggle(content[10]))
        self.my_play(LaggedStart(*[
        ...
        self.my_play(Write(content[9]), Write(content[11]))
        self.my_play(Wiggle(content[9]), Wiggle(content[11]))
...
class Scene4(MyScene):
    def construct(self):
        ...
        self.my_play(Create(triangle), Create(AH), Create(HB))
        fadein_map = (
            ((brace1, text1), (brace2, text2), (brace3, text3)),
            (UP, DR, DL)
        )
...
class Scene5(MyScene):
    def construct(self):
        ...
        brace2 = Brace(AB, AB.copy().rotate(-PI / 2).get_unit_vector())
        brace3 = Brace(AC, AC.copy().rotate(PI / 2).get_unit_vector())
        text1 = MathTex(r"\text{Rộng}", ":", "20cm", tex_template=myTemplate).scale(0.7)
        text2 = MathTex("b", "=", b).scale(0.5)
        text3 = MathTex("c", "=", c).scale(0.5)
        text4 = MathTex(r"\text{Cao}", ":", "5m").scale(0.7)
        brace1.put_at_tip(text1, buff=SMALL_BUFF)
        brace2.put_at_tip(text2, buff=SMALL_BUFF)
        brace3.put_at_tip(text3, buff=SMALL_BUFF)
        ...
        self.my_play(Create(triangle), Create(HB), Create(AH))
        fadein_map = (
            ((brace1, text1)),
            (UP)
        )
...
class Scene6(MyScene):
    def construct(self):
        ...
        tempA = np.array([50, 2000, 0])
        triangle = VMobject().set_points_as_corners([tempA, B, C, tempA])
        self.add(triangle)
        brace1 = Brace(BC, DOWN)
        brace2 = Brace(AB, AB.copy().rotate(-PI / 2).get_unit_vector())
        brace3 = Brace(AC, AC.copy().rotate(PI / 2).get_unit_vector())
        ...
        text2 = MathTex("b", "=", b).scale(0.5)
        text3 = MathTex("c", "=", c).scale(0.5)
        text4 = MathTex(r"\text{Cao}", ":", "20km").scale(0.7)
        brace1.put_at_tip(text1, buff=SMALL_BUFF)
        brace2.put_at_tip(text2, buff=SMALL_BUFF)
        brace3.put_at_tip(text3, buff=SMALL_BUFF)
        text4.move_to(ORIGIN + LEFT*1.5)
        color_map = {
            text1: RED,
            text2: GREEN,
            text3: BLUE,
...
class Scene7(MyScene):
    def construct(self):
        ...
        c = "{:.4f}".format(len_c)
        h = "{:.4f}".format(len_h)
        ha2 = "{:.6f}".format(len_h * len_a / 2)
        s = (len_a + len_b + len_c) / 2
        heron = "{:.6f}".format(1999.999999)
        ...
        self.my_play(Write(content[9]), Write(content[11]))
        self.my_play(Wiggle(content[9]), Wiggle(content[11]))
        result = MathTex(r"\text{Lệch:}", "<100", r"\text{ micro mét}",
                         tex_template=myTemplate).scale(0.8)
        result[1].set_color(YELLOW)
        result.next_to(table, DOWN)
        error = MathTex(r"\text{Sai số:}", r"<0.0000001", r"\%",
                        tex_template=myTemplate).scale(0.8) \
            .next_to(result, DOWN, buff=SMALL_BUFF)
        self.my_play(Write(result), Write(error))
